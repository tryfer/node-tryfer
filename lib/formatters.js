...
(function () {
  var formatForRestkin, formatForZipkin, base64EncodeBuffer;
  ...
  formatForZipkin = function(trace, annotations, callback) {
    ...
    tspan = new zipkinCore_types.Span({
      ...
      annotations: (thriftAnn.length > 0 ? thriftAnn : undefined),
      binary_annotations: (binaryAnn.length > 0 ? binaryAnn : undefined)
    });

    // HERE do something to base64 encode thrift stuff
    trans = new ttransport.TBufferedTransport();
