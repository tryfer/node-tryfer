...
(function () {
  ...
  formatForRestkin = function (trace, annotations, callback) {
    _formatForRestkin(trace, annotations, function(err, result) {
      if (err) {
        callback(err);
        return;
      }
   ...
   * @param {Function} callback Callback called with (err, formattedResult).
   */
  formatTracesForRestkin = function (values, callback) {
    async.map(values, function(tuple, callback) {
      var trace, annotations;

      trace = tuple[0];
      annotations = tuple[1];
      formatForRestkin(trace, annotations, callback);
    },
  ...
  if (typeof module !== 'undefined' && module.exports) {
    ...
    module.exports = {
      ...
      formatTracesForRestkin: formatTracesForRestkin,
