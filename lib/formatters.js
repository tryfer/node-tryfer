...
(function () {
  ...
  var formatForRestkin, formatForZipkin, formatForQueryService, hexStringify, ipv4ToNumber;
  var zipkinCore_types, thrift, ttransport, tprotocol, Int64;
  var _formatForRestkin, _formatForQueryService;
   ...
   */
  hexStringify = function (number) {
    var num = new Int64(number);
    return num.toOctetString();
  };
   ...
   * @param {Function} callback Callback called with (err, formattedResult).
   */
  _formatForQueryService = function (trace, annotations, callback) {
    var self = this;
    var annJson;
    var zipkinTrace = {
    ...
    if (trace.parentSpanId !== undefined) {
      zipkinTrace.parentId = trace.parentSpanId;
    }
    ...
    Array.prototype.forEach.call(annotations, function(ann, idx) {
      ...
      if (ann.annotationType === 'timestamp') {
        annJson.timestamp = ann.value;
        annJson.value = ann.name;
        zipkinTrace.annotations.push(annJson);
      } else {
        annJson.key = ann.name;
        annJson.value = ann.value.toString();
        annJson.type = ann.annotationType;
        zipkinTrace.binaryAnnotations.push(annJson);
      }
    });

    callback(null, zipkinTrace);
  };

  /**
   * Formats multiple traces and annotations for Zipkin Query Collector Service
   * @param  {[type]} trace [description]
   * @return {[type]}       [description]
   */
  formatForQueryService = function (values, callback) {
    async.map(values, function(tuple, callback) {
      var trace, annotations;

      trace = tuple[0];
      annotations = tuple[1];
      _formatForQueryService(trace, annotations, callback);
    },
  ...
  if (typeof module !== 'undefined' && module.exports) {
    ...
    module.exports = {
      ...
      formatForQueryService: formatForQueryService
    };
  } else {
