 ...
 */
function RawZipkinQueryServiceHTTPTracer(traceUrl) {
  if (traceUrl.charAt(traceUrl.length - 1) === '/') {
    traceUrl = traceUrl.slice(0, -1);
  }
...
util.inherits(RawZipkinQueryServiceHTTPTracer, EndAnnotationTracer);

RawZipkinQueryServiceHTTPTracer.prototype.performRequest = function (body, callback) {
  var url = this._traceUrl + '/api/v1/spans';
...
RawZipkinQueryServiceHTTPTracer.prototype.sendTraces = function(traces) {
  var self = this;

  async.waterfall([
    function formatTraces(callback) {
      formatters.formatForQueryService(traces, function(err, formattedTraces) {
        callback(err, formattedTraces);
      });
    },

    function makeRequest(formattedTraces, callback) {
      self.performRequest(formattedTraces, function (err, httpResp, body) {
        if (err || httpResp.statusCode >= 400) {
...
util.inherits(ZipkinQueryServiceHTTPTracer, EndAnnotationTracer);

ZipkinQueryServiceHTTPTracer.prototype.sendTraces = function (traces) {
  this._tracer.sendTraces(traces);
};
...
RawZipkinTracer.prototype._sendTrace = function(tuple, callback) {
  ...
  formatters.formatForZipkin(trace, annotations, function (err, message) {
    // scribe.Scribe.send does not take a callback and returns immediately
    callback(
      err,
      (err === null ? self.scribeClient.send(self.category, message) : null));
  });
};
 ...
 * @param {Object} options Options passed to the BufferingTracer constructor.
 */
function RESTkinScribeTracer(scribeClient, category, options) {
  var rawTracer = new RawRESTkinScribeTracer(scribeClient, category);
...
module.exports.ZipkinQueryServiceHTTPTracer = ZipkinQueryServiceHTTPTracer;
module.exports.RawZipkinQueryServiceHTTPTracer = RawZipkinQueryServiceHTTPTracer;
