...
(function () {
   ...
   *
   * @param {Hash} headers Optional.  The headers to add tracing headers to.
   ...
   * trace, and also does not support flags.
   *
   * @param {String} name The name of the request - it is highly recommended
   *    that this be the method of the request (e.g. GET, POST)
   * @param {Hash} request_headers Optional.  The headers to use when
   *    building the {Trace} - the header names *should all be lower cased*.
   *    If not provided, will assume there are no tracing headers and build a
   *    new, empty {Trace}.
   */
  Trace.fromRequest = function(name, request_headers) {
    var self = this;
    var traceOptions = {};
    ...
    ['x-b3-traceid', 'x-b3-spanid', 'x-b3-parentspanid'].forEach(function(v) {
      processedHeaders[v] = intOrNone(request_headers[v]);
    });

    // it must have both to build a span, otherwise ignore the headers
    if (processedHeaders['x-b3-traceid'] !== undefined &&
        processedHeaders['x-b3-spanid'] !== undefined) {
      traceOptions.spanId = processedHeaders['x-b3-spanid'];
      traceOptions.traceId = processedHeaders['x-b3-traceid'];
    }

    if (has(processedHeaders, 'x-b3-parentspanid')) {
      traceOptions.parentSpanId = processedHeaders['x-b3-parentspanid'];
    }

    return new Trace(name, traceOptions);
  };
