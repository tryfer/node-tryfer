...
(function () {
   ...
   * trace, and also does not support flags.
   *
   * @param {Object} request The ServerRequest object - should have the
   *    following parameters: method, headers, and socket
  ...
  Trace.fromRequest = function(request, serviceName) {
    ...
    ['X-B3-TraceId', 'X-B3-SpanId', 'X-B3-ParentSpanId'].forEach(function(v) {
      var processed = intOrNone(request.headers[v.toLowerCase()]);
      if (processed !== undefined) {
        traceOptions[v.slice(5,6).toLowerCase() + v.slice(6)] = processed;
      }
    });

    newTrace = new Trace(request.method, traceOptions);
